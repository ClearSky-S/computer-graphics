...
import ctypes
import numpy as np
g_rotation = np.zeros(3) # Euler angles
g_cam_ang = 0.
g_cam_height = .1
...
def key_callback(window, key, scancode, action, mods):
    global g_cam_ang, g_cam_height, g_rotation
    if key==GLFW_KEY_ESCAPE and action==GLFW_PRESS:
        ...
        if action==GLFW_PRESS or action==GLFW_REPEAT:
            if key==GLFW_KEY_1:
                ...
                g_rotation[0] += np.radians(10)
            elif key==GLFW_KEY_Z:
                g_rotation[0] += np.radians(-10)
            elif key==GLFW_KEY_S:
                g_rotation[1] += np.radians(10)
            elif key==GLFW_KEY_X:
                g_rotation[1] += np.radians(-10)
            elif key==GLFW_KEY_D:
                g_rotation[2] += np.radians(10)
            elif key==GLFW_KEY_C:
                g_rotation[2] += np.radians(-10)
            elif key==GLFW_KEY_V:
                g_rotation = np.zeros(3)
...
def main():
    ...
    while not glfwWindowShouldClose(window):
        ...
        yang = g_rotation[1]
        zang2 = g_rotation[2]
        Rz1 = glm.rotate(zang1, (0,0,1))
        Ry = glm.rotate(yang, (1,0,0))
        Rz2 = glm.rotate(zang2, (0,0,1))
